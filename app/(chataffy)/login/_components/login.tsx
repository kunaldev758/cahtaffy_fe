...
//         // ✅ ADD THIS
//         if (response.agents[0]?._id) {
//           sessionStorage.setItem('humanAgentId', response.agents[0]._id);
//         }
...
//           // ✅ ADD THIS - Set humanAgentId for socket connection
//           if (response.agents[0]?._id) {
//             sessionStorage.setItem('humanAgentId', response.agents[0]._id);
//           }
//           dispatchAuthStorageSync()
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-md w-full space-y-8">
          ...
          <div className="bg-white py-8 px-6 shadow-xl rounded-2xl">
            ...
            <div className="flex bg-gray-100 rounded-lg p-1 mb-4 mt-3" role="tablist">
              <button aria-selected="true" className="flex-1 py-2 px-4 rounded-md bg-white text-blue-600 font-medium text-sm shadow-sm transition-all" role="tab">
                Client Login
              </button>
              <button
                aria-selected="false"
                className="flex-1 py-2 px-4 rounded-md text-gray-500 font-medium text-sm hover:text-gray-700 transition-all"
                role="tab"
                onClick={() => {

                  console.log("checking the env :", process.env.NEXT_PUBLIC_AGENT_REDIRECTION_URL);
                  window.open(process.env.NEXT_PUBLIC_AGENT_REDIRECTION_URL, "_blank", "noopener,noreferrer");
                }}
              >
                Agent Login
              </button>
