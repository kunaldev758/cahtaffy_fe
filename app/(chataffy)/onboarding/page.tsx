...
export default function OnboardingPage() {
    ...
    // ⭐ NEW: Track if we're currently completing onboarding to prevent re-fetching
    const [isCompletingOnboarding, setIsCompletingOnboarding] = useState(false)

    // ⭐ NEW: Track if restore has already been attempted to prevent duplicate fetches
    ...
    // ⭐ FIXED: Restore onboarding step with race condition protection
    useEffect(() => {
        // ⭐ NEW: Skip if completing onboarding or already attempted restore
        if (!agentId || isCompletingOnboarding || restoreAttemptedRef.current) return

        // ⭐ NEW: Mark that we're attempting restore
        restoreAttemptedRef.current = true

        getAgentSettingsApi(agentId).then((res) => {
            if (res?.status && res?.data) {
                const { onboardingStep, onboardingWebsiteUrl, onboardingExtractedUrls, isOnboarded } = res.data

                // ⭐ NEW: If already onboarded, redirect away immediately
                if (isOnboarded) {
                ...
                // else: stay on 'source' (default)
            }
        }).catch((err) => {
            console.error('[Onboarding] Failed to restore step:', err)
            // ⭐ NEW: On error, stay on current step and let user proceed manually
        })
        // ⭐ FIXED: Dependency array - removed dateRange, added isCompletingOnboarding
    }, [agentId, isCompletingOnboarding])
    ...
    // ⭐ FIXED: handleFinish with proper error handling and state clearing
    const handleFinish = async () => {
        try {
            ...
            // ⭐ NEW: Clear onboarding-specific session data before redirecting
            if (typeof window !== 'undefined') {
                sessionStorage.removeItem('onboardingInProgress')
            }

            // ⭐ NEW: Small delay to ensure state is cleared
    ...
    // Extract a human-readable name from a URL (e.g. https://www.chataffy.com → Chataffy)
    ...
    // Case 1: Web - fetch URLs on Continue (simulate step progress during API call)
    const handleWebContinue = async () => {
        ...
        try {
            ...
            if (res?.success && Array.isArray(res.urls)) {
                ...
                setFetchStep('done')
                // Auto-set agent name from website domain
    ...
    // Case 2: Doc/Snippet - Train & Continue
    const handleDocsTrain = async () => {
      ...
      if (trimmedSnippetContent.length < 10) {
        ...
        if (!uploadedFile && !isSnippetsEnabled) {
    ...
    const handleFaqTrain = async () => {
        ...
        if (validFaqs.length === 0) {
            toast.error('Add at least one FAQ with question and answer')
            return
        }
        try {
        ...
        <div className="min-h-screen bg-[#F3F4F6] p-6">
            ...
            >
                {currentStep === 'train' ? (
                    <TrainSetup
                        urls={extractedUrls}
                        sourceDomain={(() => { try { return websiteUrl ? new URL(websiteUrl.startsWith('http') ? websiteUrl : `https://${websiteUrl}`).hostname : undefined } catch { return undefined } })()}
