...
function FieldCreationModal({ isOpen, onClose, onSave }: { isOpen: boolean; onClose: () => void; onSave: (f: any) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-[20px] bg-white p-6 mx-4">
        <div className="mb-5 flex items-center justify-between">
          <h3 className="text-[15px] font-bold text-[#111827]">Add New Field</h3>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => setFieldData({ ...fieldData, required: !fieldData.required })}
              className={`relative h-6 w-11 rounded-full transition-colors ${fieldData.required ? 'bg-[#4686FE]' : 'bg-[#E2E8F0]'}`}>
              <span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition-transform ${fieldData.required ? 'translate-x-6' : 'translate-x-1'}`} />
            </button>
          </div>
        </div>
...
function WidgetPreview({ widgetState, agentState, logoSrc }: { widgetState: WidgetState; agentState: AgentState; logoSrc: string }) {
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-[20px] shadow-[0px_8px_30px_0px_rgba(0,0,0,0.12)] w-full max-w-[320px]">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3" style={{ backgroundColor: titleBarColor }}>
        {widgetState.showLogo && (
          <img src={logoSrc} alt="logo" className="h-8 w-8 rounded-full object-cover shrink-0"
            onError={e => { (e.target as HTMLImageElement).src = '/images/widget/human-avatar.png' }} />
        )}
        <div className="flex-1 min-w-0">
          <p className="truncate text-[14px] font-semibold leading-5" style={{ color: titleBarTextColor }}>{displayName}</p>
          <div className="flex items-center gap-1">
            <span className="h-2 w-2 rounded-full bg-green-400" />
            <span className="text-[11px] opacity-80" style={{ color: titleBarTextColor }}>Online</span>
          </div>
        </div>
      </div>
      {/* Chat area */}
      <div className="flex flex-col gap-3 bg-[#F8FAFC] p-4 flex-1 min-h-[220px]">
        <div className="flex items-start gap-2">
          <div className="h-6 w-6 shrink-0 rounded-full bg-[#E2E8F0]" />
          <div className="max-w-[75%] rounded-[12px] rounded-tl-[4px] px-3 py-2 text-[12px] leading-5" style={{ backgroundColor: aiBgColor, color: aiTextColor }}>
            {widgetState.welcomeMessage || '👋 Hi there! How can I help?'}
          </div>
        </div>
        <div className="flex justify-end">
          <div className="max-w-[75%] rounded-[12px] rounded-tr-[4px] px-3 py-2 text-[12px] leading-5" style={{ backgroundColor: visitorBgColor, color: visitorTextColor }}>
      ...
      <div className="border-t border-[#E2E8F0] bg-white px-3 py-3">
        <div className="flex items-center gap-2 rounded-[8px] border border-[#E2E8F0] bg-[#F8FAFC] px-3 py-2">
          <span className="flex-1 text-[12px] text-[#94A3B8]">Type a message…</span>
          <span className="material-symbols-outlined !text-[16px] text-[#4686FE]">send</span>
        </div>
      </div>
      {!widgetState.showWhiteLabel && (
        <div className="border-t border-[#E2E8F0] bg-white py-2 text-center text-[11px] text-[#94A3B8]">
    ...
    <>
      <div className="flex gap-[20px]">
        ...
        <div className="flex flex-col gap-[20px] flex-1 min-w-0">
          ...
          <div className="rounded-[20px] bg-white shadow-[0px_4px_20px_0px_rgba(0,0,0,0.02)] p-[20px]">
            <div className="mb-4">
              ...
              <p className="text-[13px] text-[#64748B]">Customize widget colors, position and pre-chat form</p>
            </div>

            <div className="flex flex-col gap-5">
              {/* Colors */}
              <div>
                <p className="mb-3 text-[12px] font-semibold uppercase tracking-wide text-[#94A3B8]">Colors</p>
                <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                  {widgetState.colorFields.map(field => (
                    <div key={field.id} className="flex flex-col gap-1">
                      <label className="text-[11px] font-medium text-[#64748B]">{COLOR_FIELD_LABELS[field.name] ?? field.name}</label>
                      <div className="flex items-center gap-2">
                        <input type="color" value={field.value}
                          onChange={e => dispatchWidget({ type: 'UPDATE_COLOR', id: field.id, value: e.target.value })}
                          className="h-[32px] w-[32px] cursor-pointer rounded-[6px] border border-[#E2E8F0] p-0.5" />
                        <span className="text-[12px] font-mono text-[#64748B]">{field.value}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Widget Position */}
              <div>
                <p className="mb-3 text-[12px] font-semibold uppercase tracking-wide text-[#94A3B8]">Widget Position</p>
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="mb-[6px] block text-[12px] font-medium text-[#64748B]">Align</label>
                    <select value={widgetState.position.align}
                      onChange={e => dispatchWidget({ type: 'UPDATE_POSITION', payload: { align: e.target.value } })}
                      className="h-[40px] w-full rounded-[8px] border border-[#E2E8F0] bg-white px-[14px] text-[13px] text-[#111827] outline-none">
                      <option value="right">Right</option>
                      <option value="left">Left</option>
                    </select>
                  </div>
                  <div>
                    <label className="mb-[6px] block text-[12px] font-medium text-[#64748B]">Side (px)</label>
                    <input type="number" min="0" max="200" value={widgetState.position.sideSpacing}
                      onChange={e => dispatchWidget({ type: 'UPDATE_POSITION', payload: { sideSpacing: parseInt(e.target.value) || 0 } })}
                      className="h-[40px] w-full rounded-[8px] border border-[#E2E8F0] bg-white px-[14px] text-[13px] text-[#111827] outline-none" />
                  </div>
                  <div>
                    <label className="mb-[6px] block text-[12px] font-medium text-[#64748B]">Bottom (px)</label>
                    <input type="number" min="0" max="200" value={widgetState.position.bottomSpacing}
                      onChange={e => dispatchWidget({ type: 'UPDATE_POSITION', payload: { bottomSpacing: parseInt(e.target.value) || 0 } })}
                      className="h-[40px] w-full rounded-[8px] border border-[#E2E8F0] bg-white px-[14px] text-[13px] text-[#111827] outline-none" />
                  </div>
                </div>
              </div>
              ...
              {/* Additional Tweaks */}
              <div>
                <p className="mb-3 text-[12px] font-semibold uppercase tracking-wide text-[#94A3B8]">Additional Tweaks</p>
                  ...
                  {[
                    ...
                    <div key={field} className="flex items-center justify-between">
                      ...
                      <button type="button"
                        ...
                        className={`relative h-6 w-11 rounded-full transition-colors ${(widgetState as any)[field] ? 'bg-[#4686FE]' : 'bg-[#E2E8F0]'}`}>
                        <span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition-transform ${(widgetState as any)[field] ? 'translate-x-6' : 'translate-x-1'}`} />
                      </button>
                    </div>
                  ))}
                </div>
              ...
              <div>
                <div className="mb-3 flex items-center justify-between">
                  <p className="text-[12px] font-semibold uppercase tracking-wide text-[#94A3B8]">Pre-Chat Form</p>
                  <button type="button"
                    onClick={() => dispatchWidget({ type: 'SET', field: 'isPreChatFormEnabled', value: !widgetState.isPreChatFormEnabled })}
                    className={`relative h-6 w-11 rounded-full transition-colors ${widgetState.isPreChatFormEnabled ? 'bg-[#4686FE]' : 'bg-[#E2E8F0]'}`}>
                    <span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition-transform ${widgetState.isPreChatFormEnabled ? 'translate-x-6' : 'translate-x-1'}`} />
                  </button>
                </div>
                ...
                {widgetState.isPreChatFormEnabled && (
                  <div className="flex flex-col gap-2">
                    {widgetState.fields.map(field => (
                      <div key={field.id} className="flex items-center justify-between rounded-[10px] border border-[#E2E8F0] bg-[#F8FAFC] px-4 py-3">
                        <div className="flex items-center gap-3">
                          <span className="text-[16px]">{FIELD_TYPES.find(t => t.value === field.type)?.icon ?? '📝'}</span>
                          <div>
                            <p className="text-[13px] font-medium text-[#111827]">{field.value}</p>
                            <p className="text-[12px] text-[#94A3B8]">{FIELD_TYPES.find(t => t.value === field.type)?.label}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <label className="flex items-center gap-1.5 cursor-pointer">
                            <input type="checkbox" checked={field.required}
                              onChange={() => dispatchWidget({ type: 'TOGGLE_FIELD_REQUIRED', id: field.id })}
                              className="h-3.5 w-3.5 rounded accent-[#4686FE]" />
                    ...
                    <button type="button" onClick={() => setIsFieldModalOpen(true)}
                      className="flex items-center justify-center gap-2 rounded-[10px] border-2 border-dashed border-[#CBD5E1] py-3 text-[13px] font-medium text-[#64748B] hover:border-[#4686FE] hover:text-[#4686FE] transition-colors">
          ...
          <button type="button" onClick={handleSave} disabled={isSaving}
            className="inline-flex w-full min-h-12 items-center justify-center gap-2 rounded-[14px] bg-[#111827] px-[20px] text-[14px] font-semibold text-white transition-colors hover:bg-[#1f2937] disabled:opacity-60">
            {isSaving ? (
              <>
        ...
        <div className="w-[358px] shrink-0">
          <div className="sticky top-6 rounded-[20px] bg-white shadow-[0px_4px_20px_0px_rgba(0,0,0,0.02)] p-[20px]">
            <div className="mb-4">
              <h2 className="text-sm font-bold text-[#111827]">Live Preview</h2>
              <p className="text-[13px] text-[#64748B]">See how your widget will look</p>
            </div>
            <div className={`flex flex-col items-${widgetState.position.align === 'left' ? 'start' : 'end'} gap-3 rounded-[14px] bg-[#F1F5F9] p-6 min-h-[400px] relative`}>
              ...
              <div className="mt-6 w-full flex flex-col items-end gap-3">
                ...
                <div className="flex h-12 w-12 items-center justify-center rounded-full shadow-lg"
                  ...
                  <span className="material-symbols-outlined !text-[22px]" style={{ color: widgetState.colorFields[1]?.value ?? '#FFFFFF' }}>chat</span>
                </div>
              </div>
            </div>
          </div>
